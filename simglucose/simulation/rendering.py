...
class Viewer(object):
    def __init__(self, start_time, patient_name, mode="human", figsize=None):
        self.start_time = start_time
        self.patient_name = patient_name
        self.mode = mode
        if self.mode != "human":
            # Use the Agg backend for off-screen rendering
    ...
    def render(self, data):
        ...
        if self.mode == "rgb_array":
            ...
            rgb_array = np.frombuffer(self.fig.canvas.tostring_rgb(), dtype=np.uint8)
